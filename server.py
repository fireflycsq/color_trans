...
class AppState:
    def __init__(self, model_path: Path, data_dir: Path, workers: int = 2):
    ...
    def create_batch(self, name: str) -> dict:
        ...
        for child in ("input", "output", "preview"):
    ...
    def process_image(self, batch_id: str, image_id: str) -> None:
        ...
        try:
            with Image.open(root / "input" / item["input_file"]) as source:
                result = self.model.predict_image(source)
    ...
    def do_GET(self) -> None:
        try:
            ...
            if parsed.path == "/api/batches":
                ...
                mapping = {"input": ("input", "input_file"), "preview": ("preview", "preview_file"), "output": ("output", "output_file")}
                if kind not in mapping or not item[mapping[kind][1]]:
...
def main() -> None:
    ...
    args = p.parse_args()
    app = AppState(Path(args.model), Path(args.data), args.workers)
