...
默认还会把**去灰写进最终输出**：按亮度压灰雾、把实际高光分位数拉到白场（暗图会提亮）、再提中间调、温和 S 和饱和度，然后经目标 ICC 转回 CMYK。中间调默认 `--de-gray-shadow-lift 0.25`（在压黑之后，死黑不会被抬灰）。关掉用 `--no-de-gray`；对比和饱和用 `--de-gray-strength`（默认 0.6）。已生成的文件不会自动更新，需重启服务后重新跑图。
