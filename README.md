...
默认还会把**去灰写进最终输出**：CMYK→sRGB 后按亮度压黑场（灰雾拉回死黑）、温和 S 和 Lab 饱和度，再经目标 ICC 转回 CMYK。默认**不提亮暗部**（`--de-gray-shadow-lift` 为 0），否则夜景会更灰。关掉用 `--no-de-gray`；饱和与 S 用 `--de-gray-strength`（默认 0.6）。已生成的文件不会自动更新，需重启服务后重新跑图。
